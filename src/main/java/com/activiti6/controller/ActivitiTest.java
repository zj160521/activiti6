...
import com.activiti6.domain.SystemRequestParam;
import com.activiti6.domain.TestPara;
...
public class ActivitiTest {
    ...
    protected final static Logger LOGGER = LoggerFactory.getLogger(ActivitiTest.class);
    ...
    public Object test(@RequestBody SystemRequestParam<TestPara> requestParam) throws ParseException {
        ...
        deploy("审批", "vacation.bpmn");
        if (para.getProcessDefinitionId() == null ) {
            return "部署ID不能为空！";
        }
        ProcessInstance processInstance = start(para.getProcessDefinitionId(), para.getUserId());
...
//                    .taskAssignee(userId)
    ...
    private ProcessInstance start(String processDefinitionId, String userId) {
        ...
        if (instance == null) {
            ...
            instance = runtimeService.startProcessInstanceById(processDefinitionId, variableMap);
        }

        if (instance != null) {
            System.out.printf("流程实例--id: %s, name: %s", instance.getProcessDefinitionId(), instance.getProcessDefinitionName());
            System.out.println();
            return instance;
        }
        return null;
